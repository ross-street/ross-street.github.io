...
A {\it category} $C$ {\it in} a category $\CC$ is a diagram 
\begin{equation}
\xymatrix{
...
C1. $d_p d_q = d_{q-1} d_p$ for $p<q$

C2. $d_0 i = d_1 i = 1_{C_0},   d_0 i_0 = d_1 i_0 = d_1 i_1 = d_2 i_1 = 1_{C_1}$

C3. $i_0 i= i_1 i, d_2 i_0 = i d_1, d_0 i_1 = i d_0$
...
A {\it functor} $f:C\lra D$ between categories $C$ and $D$ in $\CC$ is a morphism of diagrams 
$$\xymatrix{
...
A {\it natural transformation} $\theta : f \Lra g : C \lra D$ between functors $f$ and $g$ is a morphism $$\theta : C_0 \lra D_1$$ in $\CC$ such that

N1. $d_0 \theta = f$ and $d_1 \theta = g$

N2. the following diagram commutes.
\begin{equation}
\xymatrix{
...
Consider a functor $P:\CX\lra\CC$. A morphism $x:Y\lra X$ in $\CX$ is {\it cartesian} (with respect to $P$) when the following square is a pullback for all objects $K$ of $\CX$.
\begin{equation}
\xymatrix{
...
$$
The two squares are obtained by composing vertically are equal. By hypothesis, the bottom square in each diagram is a pullback. It follows that the top square in one diagram is a pullback if and only if the top square in the other is. The result follows. \hfill{$\square$}
...
\noindent {\bf Remark}  By Lemma~\ref{cart&pb}, the pullback requirement C4 \eqref{pbc}) on a category $X$ in $\CX$ belonging to $\CX^C$ can be replaced by the requirement that $$d_0:X_2\lra X_1 \ \  \text{and} \ \   d_0:X_3\lra X_2$$ should be cartesian.
...
for the full subcategory of $\mathrm{Cat}_{/\CC}(\CX,\CY)$ consisting of those functors $F$ which preserve cartesian morphisms. Such an $F$ defines a functor $$F^C :\CX^C\lra \CY^C$$ given by $F^CX=FX$; this uses the above Remark. We obtain a functor
\begin{equation}
(-)^C: \mathrm{Cart}_{/\CC}(\CX,\CY)\lra[\CX^C,\CY^C]
\end{equation}
for each category $C$ in $\CC$.

A functor $P:\CX\lra \CC$ is called a {\it fibration} (in the sense of Grothendieck \cite{GrothDesc}, \cite{GrothFond}) when, for all $h:V\lra U$ in $\CC$ and $X$ in $\CX$ with $PX=U$, there exists a cartesian morphism $x:Y\lra X$ with $Px=h$. Such an $x:Y\lra X$ is called an {\it inverse image} of $X$ along $h$. A choice of inverse image for all such $X$ and $h$ is called a {\it cleavage} for $P$; we write $$h^X:h^{\star}X\lra X$$ for the chosen cartesian morphisms. 
...
A {\it split} fibration is a fibration equipped with a cleavage satisfying the identities
...
C_1 \ar[rr]_-{d_0} && C_0}
\end{equation}
is a pullback. Such functors $p$ are called {\it discrete opfibrations over} $C$. We think of them as internalized functors $C\lra \CC$. By Theorem~\ref{gYl}, each discrete opfibration $p:E\lra C$ corresponds to a cartesian-morphism-preserving functor 
\begin{equation}\label{pbartotal}
...
An {\it internal full subcategory} of the category $\CC$ is a discrete opfibration $p:E\lra C$ for which each of the functors $\bar{p}^U$ \eqref{pbar} is fully faithful. By Proposition~\ref{ffonfib}, this is the same as the requirement that the functor $\bar{p} : \begingroup\textstyle\int\endgroup C \lra \CC^{\mathbf{2}}$ \eqref{pbartotal} be fully faithful.
...
The 2-category $\mathrm{Cat}/\CC$ is complete. In particular, we are interested in the comma object $F/_{\CC}F^{\prime}$ of two functors over $\CC$ as in the following diagram.
\begin{equation}
\xymatrix{
\CX \ar[r]^-{F} \ar[rd]_-{P} & \CY \ar[d]^-{Q}  & \CX^{\prime} \ar[l]_-{F^{\prime}} \ar[ld]^-{P^{\prime}} \\
& \CC  }
\end{equation}
Indeed, $F/_{\CC}F^{\prime}$ is the full subcategory of the comma category $F/F^{\prime}$ consisting of those objects $(X,y,X^{\prime})$, where $X\in \CX$, $X^{\prime}\in \CX^{\prime}$ and $y\in \CY(FX,F^{\prime}X^{\prime})$, for which $Qy$ is an identity morphism in $\CC$. We have a universal square containing a natural transformation, as in the following square, taken to an identity by $Q:\CY\lra \CC$.
\begin{equation}
\xymatrix{
...
preserves comma objects.
\end{proposition}

\begin{proof} The universal property of a comma object
\begin{equation}
\xymatrix{
...
The following result is Theorem 6.2 of \cite{CoIC}. Related ideas occur earlier in \cite{Ben1975Fib} and \cite{Wraith1975}. Also see \cite{JohnstWraith}.  
...
(i) there exists an internal full subcategory $p:E\lra C$ of $\CC$ with $p_0=k$;

(ii) there exists a category $C$ in $\CC$ with $C_0=N$ and an isomorphism
...
\section{Descent categories}\label{Dm}
...
\begin{definition}\label{Desc} The {\it descent category} for a functor $P:\CE \lra \mathbf{\Delta}^{\mathrm{op}}$ is 
$$\mathrm{Desc}\CE = \CE^{\mathbf{D}} \ \ .$$
Objects $E$ of $\mathrm{Desc}\CE$ are called {\it descent data} for $P$.
\end{definition} 
...
a {\it descent object} is an object $\mathrm{Desc}T$ of $\CK$ equipped with a 2-natural isomorphism
$$\CK(K, \mathrm{Desc}T) \cong \mathrm{Desc}\CK(K,T) \ .$$
A {\it codescent object} in $\CK$ for a pseudofunctor $S: \mathbf{\Delta}^{\mathbf{op}} \lra \CK$ is a descent object for $S^{\mathrm{op}} : \mathbf{\Delta} \lra \CK^{\mathrm{op}}$. 
\end{definition}
...
A functor $P:\CX \lra \CC$ is called an {\it opfibration} (originally Grothendieck used the term `cofibration' and some authors persist with this) when $P^{\mathbf{op}}:\CX^{\mathbf{op}} \lra \CC^{\mathbf{op}}$ is a fibration. Cartesian morphisms and inverse images for $P^{\mathbf{op}}$ are called {\it opcartesian morphisms} and {\it direct images} for $P$.
...
\noindent {\bf Chevalley-Beck condition} {\it If \eqref{commsq} is a pullback in} $\CX$ {\it preserved by} $P$ {\it with} $x$ {\it cartesian and} $y$ {\it opcartesian then} $p$ {\it is opcartesian}.  
...
The Chevalley-Beck condition holds for $\mathrm{cod}:\CC^{\mathbf{2}} \lra \CC$ for any category $\CC$. To see this, notice that a square \eqref{commsq} in $\CC^{\mathbf{2}}$ is a cube \eqref{cube}.
...
\begin{example} For the moment, consider $\mathrm{Cat}$ as a category and let us look at the fibration 
...
with domain the comma category $P/\CC$ of the functor $P:\CX\lra \CC$ and $1_{\CC} : \CC \lra \CC$. 
...
\begin{example}
For any category $B$ in $\CC$, the codescent object of the simplicial object
...
The objects of $\mathrm{Spl}_{\mathrm{op}B}$ are called {\it split opfibrations over} $B$ in $\CC$.
...
Let $p:E\lra C$ be a discrete opfibration in $\CC$. Recall the definition of $\bar{p}^U :\CC(U,C)\lra \CC/U$ in Section \ref{IFSC} \label{pbar}. For each category $B$ in $\CC$, we have a morphism of diagrams
\begin{equation}
\xymatrix{
...
\begin{equation}\label{pbarcat}
\bar{p}^B :(\mathrm{Cat}\CC)(B,C) \lra \mathrm{Spl}_{\mathrm{op}B}
\end{equation}
on the descent categories. Clearly then:

\begin{proposition} If $p:E\lra C$ is an internal full subcategory of $\CC$ then, for all categories $B$ in $\CC$, the functors $\bar{p}^B$ of \eqref{pbarcat} are fully faithful.
\end{proposition}
...
In Section 5 of \cite{Weber2007} it is shown how to construct a Yoneda structure in the sense of Street-Walters \cite{StW1978} which is `good' in Weber's sense. 

If $p:E\lra C$ is an internal full subcategory of $\CC$, we say a morphism $h:V\lra U$ in $\CC$ is $C${\it -fibred} when there exist a morphism $u:U\lra C$ and a pullback square
\begin{equation}
\xymatrix{
V \ar[rr]^-{ } \ar[d]_-{h} && E \ar[d]^-{p} \\
U \ar[rr]_-{u} && C & .}
\end{equation}  
Clearly $C$-fibred morphisms are stable under pullback. 

We say the internal full subcategory $C$ {\it has coproducts} when the composite of any two composable $C$-fibred morphisms is $C$-fibred.  

\section{Internal full subcategories of $\mathrm{Cat}$}\label{IFSCoCat}

An algorithm for finding internal full subcategories of a locally presentable category $\CC$ was provided in \cite{CoIC}. This was applied to $\CC = \mathrm{Cat}$ in Section 8.10 of that paper. This
was applied to $\CC = \mathrm{Cat}$ in Section 8.10 of that paper. However, here we shall give an
...
\end{equation}
Composition is achieved by vertical pasting the squares of the form \eqref{hatsquare}.
...
The following result is related to Gray's Yoneda-like lemma on page 290 of \cite{Gray1969}; also see page 210 of Kelly's paper \cite{Kelly1974}.
...
the general theory in Section \ref{FgYl}, the functors $d_0$ and $d_1$
...
Categories in $\mathrm{Cat}$ are called {\it double categories}. They were defined by 
...
\bibitem{Kelly1974} G. Max Kelly and Ross Street, \textit{On clubs and doctrines}, Lecture Notes in Mathematics \textbf{420} (Springer-Verlag, 1974) 181--256.\label{Kelly1974}
...
\bibitem{Wraith1975} Gavin C. Wraith, \textit{Lectures on elementary topoi}. Lecture Notes in Math., \textbf{445} (Springer, Berlin, 1975) 114--206.\label{Wraith1975}
